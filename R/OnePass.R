OnePass=function(data,level,a=1.0){
  Data=as.matrix(data)
  ...
  for (k in 1:n){
    ...
    for (m in 1:c){
      ...
      size=c(1,n1)
      dist[m]=ifelse(isTRUE(all.equal(level[k],m)), (n1+1)*edist(M,size,alpha=a)/(2*(n1-1)),(n1+1)*edist(M,size,alpha=a)/(2*(n1+1)))
     }
  ...
  return(list(Clus=clus, Level=newlevel,Move=move,Within=comp$W, Between=comp$B))
}
