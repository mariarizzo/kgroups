distance_components <- function(group, distance) {
  ...
  betweens <- between_cluster(group, distance)
  ...
  B <- total - W
  list(B = B, W = W, T = total, k = k, N = N, sizes = n, 
       betweens = betweens, withins = withins)
}

between_cluster <- function(clusters, distance) {
  #  energy dissimilarity matrix between k samples or clusters
  #  should be equal to edist with method="discoB"
